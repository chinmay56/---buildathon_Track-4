...
async def get_forward_cash_forecast():
    ...
    # Pass the gross GMV float (before clawback deduction) so the forecaster can
    # model the recovery schedule itself rather than starting from an already-reduced float.
    gross_float = round(cash_pos.total_gmv_inr * 0.88, 2)
    return compute_7day_cash_forecast(
        ...
        current_settlement_float=gross_float
    )
